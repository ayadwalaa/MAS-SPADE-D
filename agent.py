...
class TemperatureAgent(object):
    def __init__(self, jid: str, password: str, verify_security: bool = False):
        ...
        self.behaviours = OneShotBehaviour.run(SupervisorAgent)
    ...
    async def _async_start(self, auto_register: bool = True) -> None:
        ...
        # Presence service
        self.presence = PresenceManager(SupervisorAgent)
    ...
    def add_behaviour(
        self, behaviour: Type[OneShotBehaviour], template: Optional[Template] = None
    ) -> None:
        """
        ...
        if issubclass(type(behaviour), OneShotBehaviour):
            for _, state in behaviour.get_states().items():
                state.set_agent(self)
...
class SupervisorAgent(object):
    def __init__(self, jid: str, password: str, verify_security: bool = False):
        ...
        self.behaviours = FSMBehaviour.run(self)
        FuzzyOutput = "API from the fuzzy controller in matlab"
        self._values = {FuzzyOutput}
